...
export default function Page() {
  const [stores, setStores] = useState<Material[]>([]);
  ...
  useEffect(() => {
    const fetchStores = async () => {
      try {
        const res = await fetch("/api/content/materials?type=Stores");
  ...
  return (
    <div className="p-6">
      {/* Title */}
      <h1 className="mb-6 select-none text-2xl font-bold text-black">Stores</h1>
        ...
        {loading
          ...
          : stores.map((item) => (
              <div
                ...
                onClick={() => {
                  router.push(
                    `/content/stores/stores-details/${item.objectId}`,
                  );
